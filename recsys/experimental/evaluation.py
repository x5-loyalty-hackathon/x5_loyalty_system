...
generator in ``recsys.experimental.model`` (both are derived from the same archetype
definitions in ``recsys.experimental.profiles``), so the absolute hit-rate-own number is
...
isolation. The product does not serve that ranking: ``recsys.experimental.service`` re-sorts
...
from recsys.experimental.contracts import ModelRecommendation, Recipe, RecommendationMode, RecommendationRequest, UserProfile
...
from recsys.experimental.model import HIGH_COVERAGE, HIGH_HISTORY_AFFINITY, NOVELTY_THRESHOLD, compute_features
...
def oracle_relevant(
    ...
    # No feasibility gate: see ``recsys.experimental.model._label_for_pair`` for why. Judging
