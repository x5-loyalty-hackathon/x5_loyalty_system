...
Их новая логика наполнения кухни пока **не перенесена**: локальное добавление
товаров там происходит до ответа сервера. Исходные коммиты и условия следующего
переноса перечислены в [чеклисте](poc-readiness.md); это не блокер текущего push.
