...
- **Статус:** API contract `1.3`, selector, книга, single-store и `cook/ready`
  ...
  2026-09-07. [Карта интеграции и адаптаций](integration-handoff.md).
...
**Историческое C6:** [серверные обои/цель](home-decoration-contract.md) остаются
в коде, но не управляют десятью предметами текущего mobile UI; выбор последних
локальный. Не считать старый контракт доказательством их серверного сохранения.
XP не тратятся и не начисляются за оформление.

**Safety 07.09:** [состав и корзина](catalog-safety.md) уточняют API 1.3:
полнота/источник готового SKU, исключения всей упаковки, однозначные закупаемые
ингредиенты. Поля необязательны; фильтрация неизвестного состава стала строже.
...
### `GET /api/v1/metrics/summary` и `GET /api/v1/metrics/daily`

Read-only аналитика синтетического ledger для будущих дашбордов, без изменения
прогресса и регистрации пользователей при чтении. Обязательные даты включительны
в Europe/Moscow, окно до 366 дней. Чеки и покупочные дни считаются отдельно,
N — параметр запроса, знаменатель — пользователи с сохранёнными событиями окна,
не участники рандомизированного пилота. [Формулы и ограничения](metrics-api.md).
Нет реальных показов UI, ITT/uplift, маржи или временного ряда реферальных XP
без соответствующих источников. Данные process-local; production auth вне PoC.
...
- user-excluded category/ingredient проверяются по всей известной упаковке,
  включая `contained_categories`; исключённая позиция чека не даёт домашнее покрытие;
- категория без явного ingredient mapping не доказывает соответствие cook-SKU;
- неоднозначные SKU нескольких обязательных недостающих ингредиентов удаляются
  до выбора магазина; полнота не обещается для несохраняемой корзины;
...
- готовый SKU должен иметь явный `meal_intent_id`; LLM не создаёт соответствие
  блюд на лету;
- при исключениях ready требует полного состава с источником производителя
  или синтетической фикстуры; аналог рецепта/неизвестный состав не подтверждают
  совместимость. Поля и ограничения — [catalog safety](catalog-safety.md);
...
Оформление хранится отдельно в том же repository (`_home_decoration_by_user`).
Каталог уровней — `app/home_decoration.py`; `GET home-decoration/{user_id}`,
`POST .../goal`, `POST .../apply` возвращают полный snapshot. Право применения
проверяется под общей блокировкой по ledger XP, без создания участника рейтинга
при чтении/изменении декора. UI хранит только последний подтверждённый snapshot
и отдельно временный предпросмотр. Перезапуск backend сбрасывает demo-состояние;
обновление страницы восстанавливает оформление из API. Поля и ошибки описаны
в [контракте C6](home-decoration-contract.md#api-и-состояние).
