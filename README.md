...
обычный чек отменены. Предел — один бонус 20 XP на покупочный день. Десять
предметов декора доступны в UI, их выбор пока локальный; [остаток до PoC](docs/poc-readiness.md).
...
- [Карта документации](docs/README.md) — действующие контракты отдельно от
  истории мерджей, промежуточной сдачи и исследовательских гипотез;
...
- [Чеклист до PoC и защиты](docs/poc-readiness.md) — оставшиеся задачи,
  критерии готовности и действительно незакрытые задачи;
- [Интеграция frontend + ML: памятка для команды](docs/integration-handoff.md) —
  PR #9/#10, API 1.3, карта кода и история проверок;
- [Состав и собираемость корзины](docs/catalog-safety.md) — исключения,
  неизвестный состав готовой еды, ограничения PoC и исправления safety-аудита;
