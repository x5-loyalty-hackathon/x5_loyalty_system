...
def build_report(
    ...
    *,
    ...
    w(
        ...
        "По повторной проверке исходной ML-ветки (`70f85dc`, 06.09.2026), "
        "это p75 `missing_count` по всем собранным (до сортировки и обрезки) "
        "кандидатам `experiment_panels(\"train\")[\"база\"]` с frozen baseline "
        "каталогом: 603 пользователя, p50=4, **p75=6**, p90=8, максимум 14. "
        "Это происхождение константы, не измерение текущего запуска. "
        "Панель не перебирает `REGIMES[:9]`; прежняя ссылка на 80 польз./мир "
        "была ошибочной. Порог откалиброван на базовом уровне дефицита, "
        "не на всех уровнях. Осознанно ниже `MAX_EFFORT_MISSING_COUNT` (8, "
