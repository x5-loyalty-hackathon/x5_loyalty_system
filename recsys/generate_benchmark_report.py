...
def build(
    ...
    w("")
    w(
        "Четыре фичи из девяти имеют |r| > 0.5. Сильнее всех — `history_affinity`: "
        "рецепт из привычных категорий это рецепт, продукты для которого уже "
        "лежат в чеке, то есть с малым `missing_count` почти по определению. "
        "Убрать effort из модели, обнулив две «effort-фичи», нельзя — он "
        "переносится остальными."
    )
    w("")
    ...
    w("")
    w("| Ранжировщик | AUC общий | AUC внутри групп |")
    w("|---|---:|---:|")
    for label, engine in _diagnostic_engines():
        ...
        w(
            f"| {label} | {signal.overall_auc:.3f} | "
            + (f"{within:.3f} |" if within is not None else "n/a |")
        )
    w("")
    w(
        "Общий AUC 0.95 выглядит как отличная модель. Внутри групп он падает "
        "почти до 0.5 — то есть вся видимая точность объясняется одной "
        "переменной, которую конвейер и так использует."
    )
    w("")
    w(
        "Корень глубже самой модели: метка оракула вырождена. В стратах с "
        "`missing_count` ≥ 6 релевантных примеров **нет вообще** — правило "
        "`max_missing_tolerance` в `recsys.profiles` делает релевантность почти "
        "синонимом effort. Модель добросовестно выучила ту метку, которую ей "
        "дали."
    )
    w("")
    w("---")
