...
_BRANDS = re.compile(
    ...
    r"handmade|балтийский берег|creme le mare|формула госта|dostaевский", re.I,
)


def clean_product_name(text: str) -> str:
    text = _BRANDS.sub(" ", text.lower().replace("ё", "е"))
...
_GROUPS = {
    ...
    "cheese": "сыр моцарел брынз", "cottage_cheese": "творо",
    ...
    "fritter": "олад", "cutlet": "котлет биточ", "meatball": "тефтел фрикадел",
...
_STOP = {latin(x) for x in (
    ...
    "натуральный натуральные рецепт рецепту филе грудка грудки кусочки "
    "начинка начинкой начинки белоногими балтийской рубленый имитированным двойной двойные"
).split()}
...
_TITLE_DECORATION = tokens("сытные быстрые авторскому рецепту ужин обед завтрак специи классика мини")
_CARD_FAMILIES = {
...
class NameIndex:
    ...
    def candidates(self, name: str, limit: int = 5) -> list[tuple[float, int]]:
        ...
        for i in candidates:
            ...
            score = .78 * coverage + .22 * precision
...
def match_product(product: dict, index: NameIndex, top_k: int = 3) -> dict:
    ...
    qfamily = family(query)
    candidates = []
    for score, i in index.candidates(product["name"], limit=20):
        recipe = index.items[i]
        title_tokens = index.token_sets[i]
        components = ingredient_tokens(recipe["ingredients"])
        ...
        missing = sorted(query - supported - (_GENERIC_DISH_MARKERS if qfamily == family(title_tokens) else set()))
        reasons = []
        ...
        extra = sorted(title_tokens - query - allowed_additions)
        if extra:
        ...
        side = re.search(r"\bс\s+(картоф\w*|рис\w*|греч\w*|макарон\w*|спагет\w*)", product["name"], re.I)
        if qfamily in {"cutlet", "meatball", "schnitzel", "nugget", "kebab", "beefstroganoff", "azu"} and side:
            recipe_side = re.search(r"\bс\s+(картоф\w*|рис\w*|греч\w*|макарон\w*|спагет\w*)", recipe["title"], re.I)
        ...
        candidates.append({"recipe_id": recipe["recipe_id"], "title": recipe["title"],
