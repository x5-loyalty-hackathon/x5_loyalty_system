...
def build() -> str:
    ...
    w("")
    w(
        "То есть колонка «Корзина» отвечает на вопрос «во что обойдётся начать готовить это "
        "блюдо, если дома нет ничего», а не «сколько стоит порция». Честная поштучная "
        "себестоимость требует норм закладки на порцию, которых у нас нет."
    )
    w("")
    ...
    w("## Сводная таблица")
    w("")
    w("| Рецепт | Тип | Кухня | Мин | Порций | Ингр. | Корзина ₽ | Готовое ₽ | Аналогов |")
    w("|---|---|---|---:|---:|---:|---:|---:|---:|")
    ...
    for dish_type in sorted(by_type):
        ...
        for recipe in sorted(by_type[dish_type], key=lambda r: r.title):
            ...
            w("")
            w(f"**Корзина целиком:** ~{basket(recipe)} ₽ (оценка)")
            w("")
            if pair:
