...
def test_verified_receipt_updates_bounded_progress_and_savings() -> None:
    ...
    assert body["reason_codes"] == ["receipt_verified", "legacy_completion_ignored"]
...
def test_concurrent_duplicate_receipt_is_recorded_once() -> None:
    ...
    assert progress_service.get_progress("user-1").avatar_xp == 0
...
def test_extra_receipt_same_day_does_not_add_purchase_day_xp() -> None:
    ...
    assert first["progress"]["avatar_xp"] == 0
    ...
    assert second["progress"]["avatar_xp"] == 0
...
def test_private_rank_is_returned_without_public_leaderboard() -> None:
    ...
    # Passive receipts and legacy completion no longer create XP. Qualify a
    # genuine referral reward for high, keeping low as a same-cohort comparison.
    post_referral(referral_request(inviter="high", invitee="low"))
    # Both receive the same reward; a second independently qualified invite
    # lets high advance without relying on the removed legacy reward bypass.
    qualify_invitee("another-invitee")
    post_referral(referral_request(inviter="high", invitee="another-invitee"))
...
def test_identity_collisions_go_to_review_instead_of_automatic_block() -> None:
    ...
    assert body["reward"]["inviter_xp"] == 0
    assert body["invitee_progress"]["avatar_xp"] == 0
