...
from recsys.experimental.evaluation import run_evaluation
...
def test_ranking_beats_the_base_relevance_rate(experimental_ml_engine) -> None:
    ...
    result = run_evaluation(experimental_ml_engine, profiles, list(RECIPES))
...
def test_the_precision_bar_is_one_a_random_ranker_fails(experimental_ml_engine) -> None:
    ...
    trained = run_evaluation(experimental_ml_engine, profiles, list(RECIPES))
    assert trained.precision_at_k_own > noise.precision_at_k_own


def test_hit_rate_flatters_a_random_ranker_but_precision_does_not(experimental_ml_engine) -> None:
...
def test_precision_lift_holds_across_seeds(experimental_ml_engine) -> None:
    """The bar is a property of the recommender, not of one population."""
    lifts = [
        run_evaluation(
            experimental_ml_engine, generate_population(ADR_001_PROFILES, seed=seed), list(RECIPES)
        ).precision_lift
...
def test_coverage_and_diversity_are_reported(experimental_ml_engine) -> None:
...
def test_evaluation_is_deterministic_given_a_seed(experimental_ml_engine) -> None:
    profiles = generate_population(15, seed=55)
    a = run_evaluation(experimental_ml_engine, profiles, list(RECIPES), seed=11)
    b = run_evaluation(experimental_ml_engine, profiles, list(RECIPES), seed=11)
...
def test_history_dependent_slice_is_reported_or_explicitly_absent(experimental_ml_engine) -> None:
